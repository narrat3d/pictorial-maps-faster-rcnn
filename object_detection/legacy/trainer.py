...
def train(create_tensor_dict_fn,
  ...
  with tf.Graph().as_default():
    ...
    import numpy as np
    print("Trainable parameters: %s" % np.sum([np.prod(v.get_shape().as_list()) for v in tf.trainable_variables()]))
    print("All parameters: %s" % np.sum([np.prod(v.get_shape().as_list()) for v in tf.all_variables()]))
    ...
    slim.learning.train(
        ...
        saver=saver) # session_wrapper=tf_debug.LocalCLIDebugWrapperSession
