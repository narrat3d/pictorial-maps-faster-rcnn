...
parameter_prefix = "DetectionBoxes_Precision/"
...
if __name__ == '__main__':
    ...
    for folder_name in folder_names:
        ...
        if (not highest_coco_values is None):
            for config_key in config.STRIDES:
                if (folder_name.find("stride%s" % config_key) != -1):
                    results_for_scales = aggregated_results.setdefault(config_key, {})
                     
                    parts = folder_name.split("_stride%s_" % config_key)
